...
export default function Breadcrumbs({ path, onNavigate }) {
    const displayPath = path.replace('/mnt/storage', '').replace(/^\//, '');
    const parts = displayPath ? displayPath.split('/') : [];
    
    return (
        <nav className="flex items-center gap-2 text-xs font-bold text-slate-400">
            <button onClick={() => onNavigate("/mnt/storage")} className="flex items-center gap-1 hover:text-blue-600 transition-colors">
                <Home size={14} /> <span>Root</span>
            </button>
            {parts.map((part, i) => {
                if (!part) return null;
                const fullPath = '/mnt/storage/' + parts.slice(0, i + 1).join('/');
                return (
